"""
COCO Dataset utilities for person and car detection
"""
...
class COCOCarPersonDataset(Dataset):
    """COCO Dataset for Person and Car detection only"""
    ...
    def __init__(self, root, split='train', transform=None, max_images=None, 
        ...
        # Filter person and car annotations
        self.annotations = defaultdict(list)
        for ann in self.coco['annotations']:
            if ann['category_id'] in [COCO_PERSON_ID, COCO_CAR_ID]:
                self.annotations[ann['image_id']].append(ann)
        
        # Only images with person or car
